           ...
           fluidPage(sidebarLayout(
             sidebarPanel(
               ...
               radioButtons("taxlevel", label = "Taxonomic level",
                            choices = c("Divisionlong", "Class", "Order", "Family", "Genus", "Species")),
               radioButtons("propchoice", label = "Proportion of group or all", choices = c("selectgroup", "All")),
           ...
           fluidPage(sidebarLayout(
             sidebarPanel(
               radioButtons("sizefract1", label = "Size fraction", 
                            choices = c("sf0.4.3", "sf3.180",  "sf3.10", "sf10.50", "sf50.200", "sf3.200"), selected = NULL),
...
server <- function(input, output, session) {
  ...
  prop_tax <- reactive({
    ...
    otutab_sfsep <- otutab_sfsep %>% select(-three200.2)
