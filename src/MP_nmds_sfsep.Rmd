...
```{r}
nmds_ten50_plot <- data.frame(cbind(ten50_nmds1$points[,1], ten50_nmds1$points[,2], meta_sf_nmds1_ten50))

#names(nmds5_plot)
names(nmds1_ten50_plot)[c(1,2)] <- c("nmds_axis1", "nmds_axis2")
```

###Plot nmds5
```{r}
ggplot(data = nmds1_ten50_plot, mapping = aes(x = nmds_axis1, y = nmds_axis2)) +
  geom_point(mapping = aes(color = Cruise, shape = depthbin)) +
